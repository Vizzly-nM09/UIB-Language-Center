...
import { Mail, Lock, Eye, EyeOff, Award, BookOpen, Globe2, Loader2 } from 'lucide-react';
...
export default function LoginPage() {
  ...
  // PERBAIKAN 1: Gunakan hook secara langsung agar reaktif
  ...
  const examFeatures = [
    { title: "TOEIC®", desc: "Communication focus.", icon: <Award className="w-4 h-4" /> },
    { title: "TOEFL ITP®", desc: "Institutional Testing.", icon: <BookOpen className="w-4 h-4" /> },
    { title: "TOEFL iBT®", desc: "Internet-based Test.", icon: <Globe2 className="w-4 h-4" /> }
  ];
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res?.error) {
        ...
        // Memberikan sedikit jeda agar user bisa melihat notifikasi sebelum pindah halaman
        setTimeout(() => {
  ...
  return (
    <div className="flex h-screen w-full bg-[#F3F4F6] font-sans overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col h-full text-white">
          ...
          <div className="max-w-md">
            <h1 className="text-4xl xl:text-5xl font-black leading-tight tracking-tight mb-7">
              {/* PERBAIKAN 2: Efek Gradient Teks */}
            ...
            <p className="text-sm xl:text-base text-white/80 font-medium leading-relaxed mb-10">
              Sistem informasi terintegrasi untuk pengelolaan sertifikasi bahasa Inggris sivitas akademika UIB.
            </p>
          </div>

          <div className="grid grid-cols-5 gap-5 mt-auto mb-6 pr-10">
            {examFeatures.map((item, index) => (
              <div key={index} className="space-y-2 group p-3 rounded-xl transition-all hover:bg-white/5">
                <div className="w-10 h-10 rounded-lg bg-white/20 flex items-center justify-center shadow-inner">
                  {item.icon}
                </div>
                <div>
                  <h3 className="text-xs font-bold tracking-tight">{item.title}</h3>
      ...
      {/* PERBAIKAN 3: Ganti bg-[#F9FAFB] ke bg-[#F3F4F6] agar kontras dengan kartu putih */}
          ...
          <div className="bg-white p-10 rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.06)] border border-white">
            ...
            <div className="mb-8">
              <h2 className="text-3xl font-black text-gray-900 tracking-tight text-center">
                {/* Efek gradient tipis pada kata Welcome */}
                <span className="bg-clip-text text-transparent bg-gradient-to-br from-gray-900 to-gray-500">
                  Welcome
                </span>
              </h2>
              <p className="text-gray-400 text-sm font-medium mt-2 text-center">Please sign in to continue</p>
            </div>
            
            <form className="space-y-5" onSubmit={handleSubmit}>
              <div className="space-y-1.5">
                <label className="text-[11px] font-black text-gray-400 ml-1 uppercase tracking-widest">Email Address</label>
              ...
              <div className="space-y-1.5">
                <label className="text-[11px] font-black text-gray-400 ml-1 uppercase tracking-widest">Password</label>
                <div className="relative group">

...
export default function UserView() {
  ...
  const { data: dataUsergroup } = useUsergroupData(
    session?.user?.accessToken,
    status,
  );
  ...
  const { mutate: createUserMutation } = useCreateUser(
    () => {
      ...
      refetch();
      setUser({ username: "", email: "", usergroup: "", isAktif: "" });
    },
    (msg) => {
      showNotification({
        status: "text-red-500",
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 items-start">
        ...
        <div className="lg:col-span-1 bg-white rounded-[2.5rem] shadow-[0_20px_50px_rgba(0,0,0,0.02)] p-8 border border-gray-100/50 sticky top-8">
          <h2 className="text-xl font-black mb-8 text-gray-900">
            {isEditing ? "Modify" : "Create"} Account
          </h2>
          <form onSubmit={handleSubmit} className="space-y-6">
            <div className="relative group">
              <input
                ...
                className="peer w-full rounded-2xl border-2 border-gray-100 px-4 pt-5 pb-2 text-sm font-bold focus:border-[#6C5DD3] focus:outline-none focus:ring-4 focus:ring-[#6C5DD3]/10 bg-white transition-all"
                placeholder=" "
              />
              <label className="absolute left-4 top-4 text-xs font-bold text-gray-400 duration-300 peer-placeholder-shown:translate-y-0 peer-placeholder-shown:scale-100 peer-focus:-translate-y-3 peer-focus:scale-75 peer-focus:text-[#6C5DD3] -translate-y-3 scale-75 origin-[0]">
                Username / NIP
              </label>
            </div>
            <div className="relative group">
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                className="w-full px-4 py-3.5 border-2 rounded-2xl border-gray-100 focus:border-[#6C5DD3] outline-none text-sm font-bold appearance-none bg-white cursor-pointer transition-all"
              >
            ...
            <div className="space-y-2">
              ...
              <select
                required
                value={user.isAktif}
                onChange={(e) => setUser({ ...user, isAktif: e.target.value })}
                className="w-full px-4 py-3.5 border-2 rounded-2xl border-gray-100 focus:border-[#6C5DD3] outline-none text-sm font-bold appearance-none bg-white cursor-pointer transition-all"
              >
                <option value="" disabled>
                  Choose Status...
            ...
            <div className="flex gap-2 pt-4">
              <button
                ...
                className="flex-1 py-4 font-black text-[10px] text-gray-400 uppercase tracking-widest hover:text-red-500 transition-all"
              >
                Reset
              </button>
              <button

...
interface DataTableProps<TData, TValue> {
  ...
  catchChangedModul?: (modulData: any) => void;
  handleSubmit?: () => void;
}
...
export function DataTable<TData, TValue>({
  ...
  const table = useReactTable({
    ...
    getSubRows: (row: any) => row.SubModul || [],
    initialState: {
      pagination: {
        pageIndex: 0,
        pageSize: 10,
  ...
  useEffect(() => {
    ...
    data.forEach((item: any, index) => {
      selectedRows[item.modul_id] = item.IsChecked === "y" ? true : false;
      const isSubModulExist = item.SubModul && item.SubModul.length > 0;
      if (isSubModulExist) {
        item.SubModul.forEach((subItem: any) => {
          selectedRows[subItem.modul_id] =
            subItem.IsChecked === "y" ? true : false;
        });
      }
    });

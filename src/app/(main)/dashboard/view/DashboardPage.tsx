...
export default function DashboardView() {
  ...
  const trendData = useMemo(() => {
    ...
    return {
      ...
      datasets: [
        {
          label: "TOEIC (Avg)",
          data: monthlyStats.TOEIC.map((m) =>
            m.count > 0 ? Math.round(m.total / m.count) : 0,
          ),
  ...
  const currentCategoryStats = useMemo(() => {
    ...
    const passCount = group.filter((s: any) =>
  ...
  const mainStats = useMemo(() => {
    const calcAvg = (key: string) => {
      const group = studentData.filter((s: any) =>
        s.jenis_test?.toUpperCase().includes(key),
      );
      return group.length
        ? Math.round(
            group.reduce((a: any, b: any) => a + (b.TotalScore || 0), 0) /
              group.length,
          )
        : 0;
    };
        ...
        <div className="flex flex-col lg:flex-row gap-4 items-end border-t border-gray-50 pt-6">
          <div className="w-full lg:flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <select
                className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-700 outline-none focus:border-[#9969ff] transition-all"
                value={filterInputs.prodi}
                onChange={(e) =>
            ...
            <div className="space-y-1">
              ...
              <select
                className="w-full px-4 py-2.5 bg-gray-50 border border-gray-200 rounded-xl text-sm font-bold text-gray-700 outline-none focus:border-[#9969ff] transition-all"
                value={filterInputs.tahun}
                onChange={(e) =>
                  setFilterInputs({ ...filterInputs, tahun: e.target.value })
                }
              >
        ...
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
            <StatCard
              ...
              value={mainStats.total}
              icon={Icons.Users}
              color="bg-blue-500"
            />
            <StatCard
              title="Avg TOEIC"
              value={mainStats.toeicAvg}
              icon={Icons.Score}
              color="bg-[#9969ff]"
            />
            <StatCard
              title="Avg TOEFL"
              value={mainStats.toeflAvg}
              icon={Icons.Test}
              color="bg-orange-500"
            />
            <StatCard
            ...
            />
            <StatCard
              title={`Failed (${passFailCategories[chartIndex].title})`}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            ...
            <div className="lg:col-span-2 bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm">
              ...
              <div className="w-full aspect-[21/9] min-h-[300px]">
            ...
            <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm flex flex-col items-center">
              <div className="flex justify-between items-center w-full mb-8">
                <div className="flex items-center gap-2 text-gray-800 font-bold text-[10px] uppercase tracking-tight">
                  <PieChart size={18} className="text-[#9969ff]" />{" "}
                  {currentCategoryStats.title}
                </div>
                <div className="flex gap-1">
                  <button
                    onClick={() =>
                      setChartIndex((prev) => (prev === 0 ? 3 : prev - 1))
                    }
                    className="p-1.5 rounded-lg bg-gray-50 hover:bg-[#9969ff] hover:text-white transition-all"
                  >
                    <ChevronLeft size={14} />
                  </button>
                  <button
                    onClick={() =>
                      setChartIndex((prev) => (prev === 3 ? 0 : prev + 1))
                    }
                    className="p-1.5 rounded-lg bg-gray-50 hover:bg-[#9969ff] hover:text-white transition-all"
                  >
                    ...
                    <Doughnut
                      data={{
                        ...
                        datasets: [
                          {
                            data: [
                              currentCategoryStats.passCount,
                              currentCategoryStats.failCount,
                            ],

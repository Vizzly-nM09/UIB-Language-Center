...
export default function StudentDetailPage({
  ...
  const history = useMemo(
    () => studentData.filter((m: any) => m.npm_mahasiswa === npm),
    [studentData, npm],
  );

  if (isLoading)
    return (
      <div className="p-20 text-center animate-pulse font-black text-gray-400 uppercase tracking-widest">
        Student Data Synchronization...
      </div>
    );
  if (!history.length)
    return (
      <div className="p-20 text-center bg-white m-10 rounded-3xl border border-dashed text-red-400 font-bold uppercase tracking-widest">
        Data #{npm} Can't Be Found
      </div>
    );
    ...
    <div className="min-h-screen w-full p-6 font-sans bg-[#F8F9FC] text-gray-900 pb-20">
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="flex justify-between items-center bg-white p-4 rounded-2xl border border-gray-100 shadow-sm">
          <button
            onClick={() => router.back()}
            className="flex items-center gap-2 px-4 py-2 rounded-xl hover:bg-gray-50 text-gray-500 font-black text-[12px] uppercase tracking-widest transition-all"
          >
            <ArrowLeft size={16} /> Return
          </button>
          <div
            className={`px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-tighter flex items-center gap-2 ${isPassed ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600"}`}
          >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 bg-[#6C5DD3] rounded-[2.5rem] p-10 text-white relative overflow-hidden shadow-xl shadow-purple-100">
            <div className="relative z-10">
              ...
              <h1 className="text-4xl font-black tracking-tight mb-2 uppercase">
                {latest.nama_mahasiswa}
              </h1>
              <p className="text-white/70 font-bold tracking-[0.2em] text-[10px] uppercase">
                Nomor Pokok Mahasiswa / {latest.npm_mahasiswa}
              </p>

              <div className="mt-10 flex flex-wrap gap-10 border-t border-white/10 pt-8">
                {[
                  { label: "Last Exam", value: latest.jenis_test },
                  { label: "Test Type", value: latest.tipe_test },
                  { label: "Date", value: latest.tanggal_ujian },
                ].map((info, i) => (
                  <div key={i}>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 border border-gray-100 shadow-sm flex flex-col justify-center items-center text-center">
            <p className="text-gray-400 text-[10px] font-black uppercase tracking-[0.2em] mb-4">
              Total Score of {latest.jenis_test} ({latest.tipe_test})
            </p>
            <div className="text-8xl font-black text-gray-800 tracking-tighter tabular-nums">
              {latest.TotalScore}
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {[
            ...
            <div
              key={idx}
              className="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm flex items-center gap-6 group hover:border-[#6C5DD3] transition-all duration-300"
            >
              <div
                className={`p-4 rounded-2xl ${item.bg} ${item.color} shadow-sm group-hover:scale-110 transition-transform`}
              >
                <item.icon size={22} />
              </div>
              <div>
                ...
                <p className="text-2xl font-black text-gray-800 tracking-tighter mt-1">
                  {item.value || 0}
                </p>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          <div className="bg-white p-8 rounded-[2.5rem] border border-gray-100 shadow-sm flex flex-col">
            <div className="flex items-center gap-2 mb-8 text-[#6C5DD3] font-black text-xs uppercase tracking-widest">
              <TrendingUp size={16} /> Score Progression Chart
            </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-gray-100 shadow-sm overflow-hidden flex flex-col">
            <div className="p-6 border-b border-gray-50 font-black text-xs uppercase tracking-widest text-gray-800 flex items-center gap-2">
              <FileText size={16} /> Complete Exam History
            </div>
              ...
              <Table>
                <TableBody>
                  {history.map((h: any) => (
                    ...
                    >
                      <TableCell className="px-8 py-5 text-xs font-bold text-gray-500 tracking-tighter">
                        {h.tanggal_ujian}
                      </TableCell>
